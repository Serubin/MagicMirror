...
jQuery(document).ready(function($) {
	...
	function updateCalendarData() {
		new ical_parser("calendar.php", function(cal) {
			...
			for (var i in events) {
				...
				if(days > calendar_display_limit)
					continue;
				...
				if (e.RRULE) {
					...
					var dates = rule.between(new Date(), endDate, true, function(date, i) {
						return i < 10
					});
					for (date in dates) {
	...
	function updateCurrentWeather() {
		...
		$.getJSON('weather.php?action=weather', weatherParams, function(json, textStatus) {
	...
	function updateWeatherForecast() {
		...
		$.getJSON('weather.php?action=forecast', weatherParams, function(json, textStatus) {
	...
	setTimeout(function(){
		checkVersion(); // starts auto-update reloader
